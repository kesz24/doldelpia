...
class curl {
  var $ch, $agent, $error, $info, $cookiefile, $savecookie; 
    function curl() {
        ...
        $this->ch = curl_init();
        curl_setopt ($this->ch, CURLOPT_USERAGENT, "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_2) AppleWebKit/535.7 (KHTML, like Gecko) Chrome/16.0.912.63 Safari/535.7");
    ...
    function getPagenoheader($url, $hasHeader=false, $hasBody=true) {
        curl_setopt($this->ch, CURLOPT_HEADER, $hasHeader ? 1 : 0);
        curl_setopt($this->ch, CURLOPT_NOBODY, $hasBody ? 0 : 1);
        curl_setopt ($this->ch, CURLOPT_URL, $url);
        $data = curl_exec ($this->ch);
...
function getStr2($string2,$start2,$end2){
    $str2 = explode($start2,$string2);
    $str2 = explode($end2,$str2[2]);
    return $str2[0];
}
